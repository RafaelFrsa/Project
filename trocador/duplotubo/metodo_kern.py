...
def yut(fluido1,fluido2,material):
    ...
    result_fl={u'Vazao':(str(fluido1['Vazao'])+u' Kg/s',str(fluido2['Vazao'])+u' Kg/s'),
                u'Temperatura de Entrada':(str(fluido1['T_entr'])+u' ºC',str(fluido2['T_entr'])+u' ºC'),
                u'Temperatura de Saida':(str(fluido1['T_said'])+u' ºC',str(fluido2['T_said'])+u' ºC'),
                u'Velocidade Media de Escoamento':(str(fluido1['Vel_m'])+u' m/s',str(fluido2['Vel_m'])+u' m/s'),
                u'Numero de Reynolds':(str(fluido1['Re'])+u' ',str(fluido2['Re'])+u' '),
                u'Numero de Nusselts':(str(fluido1['Nu'])+u' ',str(fluido2['Nu'])+u' '),
                u'Coeficiente de Pelicula (h)':(str(h_i)+u' W/m2.K',str(h_o)+u' W/m2.K')};
    result_pres={u'DP':(str(fluido1[u'\u0394P'])+u' Pa',str(fluido2[u'\u0394P'])+u' Pa'),
                u'DPrb':(str(material[u'\u0394Pr1'])+u' Pa',str(material[u'\u0394Pr2'])+u' Pa'),
                u'DPtotal':(str(fluido1[u'\u0394Ptotal'])+u' Pa',str(fluido2[u'\u0394Ptotal'])+u' Pa'),
                u'Potencia de Bombeamento':(str(fluido1['Potencia_bomb'])+u' W',str(fluido2['Potencia_bomb'])+u' W'),
                u'DPnl (bocais)':(str('---'),str(material[u'\u0394Pn']))}
    result_geral={u'Area de Troca Termica Total':(str(area)),
                u'Calor Trocado (Heat Duty)':(str(q)),
                u'Coef. de Trans. de Calor Limpo (Uc)':(str(U_c)),
                u'Coef. de Trans. de Calor Incrustado (Ud)':(str(U_d)),
                u'Fator de Limpeza (CF)':(str(CF)),
                u'Var. Log. de Temperatura (dTm)':(str(dTm)+u' K'),
                u'Area por Grampo Tubular':(str(area_grampo)+u' m2'),
                u'Numero de Grampos':(str(num_grampo)+u' grampo(s)'),
                u'Excesso de Area (Over-Surface Design)':(str(OS)+u'%')}
